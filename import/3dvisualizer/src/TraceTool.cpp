/***********************************************************************
TraceTool - Tool to create and edit 3D curves.
...
/*************************************
Methods of class TraceToolFactory:
*************************************/

TraceToolFactory::TraceToolFactory(ToolManager& toolManager)
	:ToolFactory("TraceTool",toolManager),
	 detailSize(getUiSize()),
	 curveFileName("TraceTool.curves")
	{
	...
	TraceTool::factory=this;
	}

TraceToolFactory::~TraceToolFactory(void)
	{
	/* Reset tool class' factory pointer: */
	TraceTool::factory=0;
	}

const char* TraceToolFactory::getName(void) const
	{
	return "Curve Editor";
	}

const char* TraceToolFactory::getButtonFunction(int) const
	{
	return "Draw Curves";
	}

Tool* TraceToolFactory::createTool(const ToolInputAssignment& inputAssignment) const
	{
	return new TraceTool(this,inputAssignment);
	}

void TraceToolFactory::destroyTool(Tool* tool) const
	{
	delete tool;
	}

extern "C" void resolveTraceToolDependencies(Plugins::FactoryManager<ToolFactory>& manager)
	{
	/* Load base classes: */
	manager.loadClass("UtilityTool");
	}

extern "C" ToolFactory* createTraceToolFactory(Plugins::FactoryManager<ToolFactory>& manager)
	{
	/* Get pointer to tool manager: */
	ToolManager* toolManager=static_cast<ToolManager*>(&manager);
	
	/* Create factory object and insert it into class hierarchy: */
	TraceToolFactory* TraceToolFactory2=new TraceToolFactory(*toolManager);
	
	/* Return factory object: */
	return TraceToolFactory2;
	}

extern "C" void destroyTraceToolFactory(ToolFactory* factory)
	{
	delete factory;
	}

/**************************************
Static elements of class TraceTool:
**************************************/

TraceToolFactory* TraceTool::factory=0;
const TraceTool::Curve::Color TraceTool::curveColors[8]=
	{
	TraceTool::Curve::Color(0,0,0),TraceTool::Curve::Color(255,0,0),
	TraceTool::Curve::Color(255,255,0),TraceTool::Curve::Color(0,255,0),
	TraceTool::Curve::Color(0,255,255),TraceTool::Curve::Color(0,0,255),
	TraceTool::Curve::Color(255,0,255),TraceTool::Curve::Color(255,255,255)
	};

/******************************
Methods of class TraceTool:
******************************/

TraceTool::TraceTool(const ToolFactory* sFactory,const ToolInputAssignment& inputAssignment)
	...
	controlDialogPopup=new GLMotif::PopupWindow("TraceToolControlDialog",getWidgetManager(),"Curve Editor Settings");
	...
	lineWidthSlider->getValueChangedCallbacks().add(this,&TraceTool::lineWidthSliderCallback);
	...
	for(int i=0;i<8;++i)
		...
		colorButton->getSelectCallbacks().add(this,&TraceTool::colorButtonSelectCallback);
		}
	...
	curvesSelectionHelper.addSaveCallback(saveCurvesButton,this,&TraceTool::saveCurvesCallback);
	
	GLMotif::Button* loadCurvesButton=new GLMotif::Button("LoadCurvesButton",buttonBox,"Load Curves...");
	curvesSelectionHelper.addLoadCallback(loadCurvesButton,this,&TraceTool::loadCurvesCallback);
	
	GLMotif::Button* deleteAllCurvesButton=new GLMotif::Button("DeleteAllCurvesButton",buttonBox,"Delete All Curves");
	deleteAllCurvesButton->getSelectCallbacks().add(this,&TraceTool::deleteAllCurvesCallback);
	
	GLMotif::Button* deleteLastCurveButton=new GLMotif::Button("DeleteLastCurveButton",buttonBox,"Delete Last Curve");
	deleteLastCurveButton->getSelectCallbacks().add(this,&TraceTool::deleteLastCurveCallback);
...
void TraceTool::buttonCallback(int,InputDevice::ButtonCallbackData* cbData)
	{
	...
	if(cbData->newButtonState)
		...
		//std::cout << dataSetRenderer->getStepSize() << std::endl;
...
void TraceTool::display(GLContextData& contextData) const
	{
	/* Set up OpenGL state: */
...
void TraceTool::lineWidthSliderCallback(GLMotif::Slider::ValueChangedCallbackData* cbData)
	{
...
void TraceTool::colorButtonSelectCallback(GLMotif::NewButton::SelectCallbackData* cbData)
	{
	/* Set the new line color: */
	newColor=curveColors[colorBox->getChildIndex(cbData->button)];
	}

void TraceTool::saveCurvesCallback(GLMotif::FileSelectionDialog::OKCallbackData* cbData)
	{
	if(isMaster())
		{
		try
...
void TraceTool::loadCurvesCallback(GLMotif::FileSelectionDialog::OKCallbackData* cbData)
	{
	...
	if(!curvesSource.isString("Vrui Curve Editor Tool Curve File"))
		Misc::throwStdErr("TraceTool::loadCurvesCallback: File is not a curve file");
...
void TraceTool::deleteAllCurvesCallback(Misc::CallbackData* cbData)
	{
	/* Deactivate the tool just in case: */
	active=false;
...
void TraceTool::deleteLastCurveCallback(Misc::CallbackData* cbData)
	{
	/* Deactivate the tool just in case: */
	active=false;
